...
public class MessageListenerSink extends AbstractQueuedCallbackSink {
    ...
    private Multimap<Class<? extends Measurement>,
            Measurement.Listener> mMeasurementListeners = HashMultimap.create();
    private Multimap<String, DiagnosticResponse.Listener> mDiagnosticListeners = HashMultimap.create();
    private Map<String, DiagnosticRequest> mRequestMap = new HashMap<>();

    public MessageListenerSink() {
        mMeasurementListeners = HashMultimap.create();  //TODO isn't this redundant? it's initialized above. same for diagnosticListeners
        mMeasurementListeners = Multimaps.synchronizedMultimap(mMeasurementListeners);
        mDiagnosticListeners = HashMultimap.create();
        mDiagnosticListeners = Multimaps.synchronizedMultimap(mDiagnosticListeners);
    }
    ...
    public void registerUntilDone(DiagnosticRequest request,
        ...
        //sending a request with the same diagnosticIdentifier as a previous one cancels the last one, so just overwrite it
        mRequestMap.put(diagnosticIdentifier, request);
        mDiagnosticListeners.put(diagnosticIdentifier, listener);
        //TODO do we need to send the last known to the sink like in the other register method? 
        //I don't think so, that probably wouldn't make sense for something query based like this
    }
        ...
        if(message instanceof SimpleVehicleMessage) {
            try {
                ...
                for(Measurement.Listener listener :
                        mMeasurementListeners.get(BaseMeasurement.getClassForId(((SimpleVehicleMessage) message).getName()))) {
            ...
            DiagnosticResponse response = (DiagnosticResponse) message;
